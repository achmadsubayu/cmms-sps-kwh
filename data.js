    ...
    .then(data => {
        if(data && data.length > 0) {
            ...
            data.forEach(d => {
                ...
                // UBAHAN: Format menjadi Jam saja (HH:00)
                let label = dt.getHours().toString().padStart(2, '0') + ':00';
                if (!hourlyMap[label]) hourlyMap[label] = { sum: 0, count: 0 };
                hourlyMap[label].sum += parseFloat(d.speed) || 0;
                hourlyMap[label].count += 1;
            });

            tampilanTimeLabels = Object.keys(hourlyMap);
            // Hitung rata-rata per jam dari InfluxDB
            tampilanSpeedData = tampilanTimeLabels.map(l => Number((hourlyMap[l].sum / hourlyMap[l].count).toFixed(2)));
            ...
            if (tampilanTimeLabels.length > 0) {
                ...
                currentHourSpeedSum = hourlyMap[currentHourLabel].sum;
                currentHourSpeedCount = hourlyMap[currentHourLabel].count;
            }
...
function liveUpdateDashboard() {
    ...
    if(document.getElementById('page-tampilan').classList.contains('active') && currentMachine) {
        ...
        // (Tetap pakai 20, tapi sekarang artinya 20 jam ke belakang di chart)
        if (isLiveView) {
            tampilanSpeedChartInstance.options.scales.x.min = Math.max(0, tampilanTimeLabels.length - 20);
            tampilanSpeedChartInstance.options.scales.x.max = tampilanTimeLabels.length - 1;
            let btn = document.getElementById('btnLiveView');
...
function calculateAndAddSchedule() {
    ...
    // --- PERBAIKAN MUTLAK ACTUAL OUTPUT ---
    // Semua jadwal baru WAJIB dimulai dari 0 agar tidak mewarisi angka dari shift/jadwal sebelumnya.
    // Sistem delta (penambah) di pollRealtimeData yang akan menghitung kenaikannya (+1, +2, dst).
    let initialActual = 0;
    ...
    .then(res => res.json())
    .then(dataFB => {
        // Tangkap key yang baru dibuat dari Firebase
...
function updateScheduleInline(index, field, value) {
    ...
    // Menghindari kekacauan pada perhitungan Delta actual realtime output
    if(field === 'actual') {
         if(machineData[mac]) {
